...
2. **`ConditionChip`** — SF Symbol + value in `.footnote` on a condition-tinted glass capsule (tint on the icon + glass wash, text `.primary` — palette-colored text is unreadable over bright map imagery; Reduce Transparency → thick material). Max 4 per card: wind (e.g. "↘ tailwind home"), temp+sky, travel ("45m away"), duration ("~3h ride").
