...
///
/// ponytail: builds its own in-memory container inline rather than pulling
/// in `RideOnModelContainer` from Services — Models can't depend on Services
/// (Services depends on Models), and this test only needs a Schema of the
/// three `@Model` types anyway.
@MainActor
final class SwiftDataRoundTripTests: XCTestCase {
    private func makeInMemoryContext() throws -> ModelContext {
        let schema = Schema([RouteModel.self, RideLogModel.self, SavedPlaceModel.self])
