...
/// The persisted, CloudKit-mirrored route record. Deliberately app-layer
/// (not `RideOnCore`) — `RideOnCore.Route` is the platform-free value type
/// the scoring engine works with; this is SwiftData's on-disk shape, richer
/// (raw geometry, import provenance) than the engine needs.
///
...
final class RouteModel {
    ...
    /// material (Phase 3).
    var bearingSegmentsData: Data?

    var createdAt: Date = Date.now
    var sourceRaw: String = RouteSource.gpxImport.rawValue
    var stravaRouteID: String?
