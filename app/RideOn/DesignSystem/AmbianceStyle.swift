...
enum SkyCondition: String, CaseIterable, Sendable, Equatable {
    case sunny
    case overcast
    case rain
    case night
...
enum AmbianceStyle {
    ...
    static func resolvedCondition(sky: SkyCondition, date: Date) -> SkyCondition {
        isNight(date) ? .night : sky
    }

    static func gradient(sky: SkyCondition, date: Date) -> LinearGradient {
        gradient(for: resolvedCondition(sky: sky, date: date))
    }

    static func gradient(for condition: SkyCondition) -> LinearGradient {
        switch condition {
        case .sunny:
            LinearGradient(
...
struct AmbianceBackground: View {
